...
keiba-prediction は、中央競馬の重賞レース（GI/GII/GIII）を対象とした予想システムです。14のエージェントがパイプライン構成で連携し、データ取得 → 特徴量生成 → 統計分析 → Web調査 → 根拠統合 → オッズ評価 → 予想生成 → バックテスト → Note記事生成 → 品質保証までを一貫して自動実行します。
...
現在のバージョン（MVP）では、外部サイトへのアクセスを行わず、**サンプルデータ（架空のGIレース・10頭立て）** のみで動作確認できます。本番データへの差し替えは `DataSource` を実装することで対応可能です（[セクション8参照](#8-データソースの差し替え)）。
...
2. **エージェント実行ログ**: 14エージェントの実行結果（✅/❌ と所要時間）
...
| 6 | `web_research` | WebResearcher | `current_data` | `parallel_1` |
| 7 | `evidence` | EvidenceIntegrator | `python_analysis`, `web_research` | — |
| 8 | `predicted_odds` | PredictedOddsEvaluator | `evidence` | — |
| 9 | `actual_odds` | ActualOddsEvaluator | `predicted_odds` | — |
| 10 | `prediction` | PredictionGenerator | `actual_odds` | — |
| 11 | `backtest` | Backtester | `prediction` | — |
| 12 | `note_research` | NoteStructureResearcher | `prediction` | — |
| 13 | `note_write` | NoteWriter | `note_research` | — |
| 14 | `qa` | QualityAssurance | `note_write` | — |

### PipelineContext（共有状態）

全14エージェントは `PipelineContext` オブジェクトを共有します。各エージェントは前段の出力を読み取り、自身の結果を書き込みます。
...
PipelineContext:
  ...
  features                # Agent 4
  analysis                # Agent 5
  web_research            # Agent 6
  evidence                # Agent 7
  predicted_odds_eval     # Agent 8
  actual_odds_eval        # Agent 9
  prediction_predicted    # Agent 10
  prediction_actual       # Agent 10
  backtest                # Agent 11
  note_suggestion         # Agent 12
  note_article            # Agent 13
  qa_report               # Agent 14
...
Agent 5（PythonAnalyzer）と Agent 6（WebResearcher）は互いに依存しないため、`ThreadPoolExecutor` で並列実行されます。各スレッドには `PipelineContext` のコピー（`deepcopy`）が渡され、実行後に結果がメインの Context にマージされます。

### QAゲートとリトライ

Agent 14（QualityAssurance）が品質をチェックし、**100点未満**の場合は該当するエージェントに差し戻されます：
...
| Web調査の信頼性 | `web_researcher`（Agent 6） |
| オッズ期待値 | `actual_odds_evaluator`（Agent 9） |
| バックテスト結果 | `backtester`（Agent 11） |
| 券種別予想の妥当性 | `prediction_generator`（Agent 10） |
| Noteの読みやすさ | `note_writer`（Agent 13） |
| 誇大表現の排除 | `note_writer`（Agent 13） |
| リスク説明 | `note_writer`（Agent 13） |
...
### 5.8 PredictedOddsEvaluator（予想オッズ評価）
...
### 5.9 ActualOddsEvaluator（実オッズ評価）
...
### 5.12 NoteStructureResearcher（Note構成調査）

**役割:** 競馬予想Note記事の構成を提案するエージェント
...
- `context.note_suggestion` — 記事構成提案：
  - `suggested_title` — タイトル案（例: `【GI予想】第93回東京優駿 データ分析×期待値評価`）
  - `structure` — 9セクションの構成案
  - `tone` — 記事のトーン（`analytical_calm`）
  - `successful_patterns` — 成功パターンのヒント
  - `ng_expressions` — 使用禁止表現（9種）
...
| 3 | データ分析結果 | スコア・勝率推定 |
| 4 | 注目馬ピックアップ | 本命・対抗・穴 |
| 5 | 期待値評価 | 予想オッズ vs 実オッズ |
| 6 | 買い目提案 | 券種別 |
| 7 | 見送り判定 | その理由 |
| 8 | リスク説明・免責事項 | 注意喚起 |
| 9 | まとめとチェックリスト | 当日更新項目 |

**データソース:** なし

---

### 5.13 NoteWriter（Note作成）

**役割:** 予想結果をもとにNote投稿用のMarkdown記事案を作成するエージェント
...
- `context.note_article` — 記事データ：
  ...
  - `summary_box` (str) — サマリー（本命/対抗/穴/見送り）
...
1. 9セクションのMarkdown本文を構築：
   - **導入**: レース名・グレード・条件の表示、免責
   - **出走馬評価一覧**: 順位表（グレードアイコン 🔴🟠🟡⚪ 付き）
   - **データ分析結果**: 上位5馬の強み・懸念
   - **注目馬ピックアップ**: 本命・対抗・穴馬
   - **期待値評価**: オッズとEVの表示（📈/📉）
   - **買い目提案**: 券種別の推奨（リスクアイコン 🟢🟡🔴 付き）
   - **見送り判定**: 見送り推奨の場合に理由を表示
   - **リスク説明**: 6項目の免責事項
   - **チェックリスト**: 当日確認すべき6項目
2. **禁止表現チェック**: 9種の禁止語をスキャンし、検出結果を記録
...
| `agent_results` | 14エージェントの実行ログ（成功/失敗・所要時間） |
...
```yaml
data_source:
  active: "sample"               # "sample"（MVP）または "production"（将来）
  sample:
    race_id: "20260607-Tokyo-11" # サンプルレースID
  production:
    historical_api: ""           # 本番用API（将来実装）
    current_api: ""
    web_search_engine: ""
```
...
| `get_predicted_odds(race_id)` | 予想オッズ | Agent 8 |
| `get_actual_odds(race_id)` | 実オッズ | Agent 9 |
| `get_web_content(race_id, horse_ids)` | Web調査結果 | Agent 6 |
| `get_backtest_data(config)` | バックテスト用過去データ | Agent 11 |
...
# エージェント別テスト
pytest tests/test_agents/test_agents_pipeline.py -v    # Agent 1-6
pytest tests/test_agents/test_agents_7_14.py -v         # Agent 7-14
